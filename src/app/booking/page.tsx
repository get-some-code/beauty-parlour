...
function CustomCalendar({ value, onChange, error }: {
  value: string;
  ...
  return (
    <div aria-label="Date picker" role="group"
      style={{ border: `1px solid ${error ? "rgba(239,68,68,0.5)" : "rgba(201,168,76,0.18)"}`, borderRadius: "1rem", background: "rgba(255,255,255,0.03)", overflow: "hidden" }}>
      {/* Month nav */}
      <div className="flex items-center justify-between px-4 py-3"
        ...
        <p className="font-serif font-bold text-sm" style={{ color: "rgba(237,224,196,0.8)" }}>
      ...
      <div className="grid grid-cols-7 px-3 pt-3 pb-1">
        {DAYS.map(d => (
          <div key={d} className="text-center text-[9px] uppercase tracking-widest font-sans font-semibold pb-2"
            style={{ color: "rgba(201,168,76,0.4)" }}>{d}</div>
        ))}
      </div>
        ...
        {cells.map((day, i) => {
          ...
          return (
            <button key={day}
              onClick={() => select(day)}
              disabled={past}
              aria-label={`${day} ${MONTHS[viewMonth]} ${viewYear}${selected ? ", selected" : ""}${past ? ", unavailable" : ""}`}
              aria-pressed={selected}
              className="relative mx-auto w-9 h-9 flex items-center justify-center rounded-full text-xs font-sans font-medium transition-all duration-200
                         disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C9A84C]"
              style={{
                background: selected
                  ? "linear-gradient(135deg,#C9A84C,#E8D5A3)"
                  : tod ? "rgba(201,168,76,0.12)" : "transparent",
                color: selected ? "#080604" : past ? "rgba(237,224,196,0.18)" : tod ? "rgba(201,168,76,0.9)" : "rgba(237,224,196,0.65)",
                fontWeight: selected || tod ? 700 : 400,
              }}>
              {day}
              {tod && !selected && (
                <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full"
                  style={{ background: "#C9A84C" }} aria-hidden="true" />
              )}
            </button>
          );
        })}
...
function CustomTimePicker({ value, onChange, error }: {
  ...
  return (
    <div role="group" aria-label="Select appointment time">
      <div className={`rounded-2xl p-1 grid grid-cols-2 sm:grid-cols-2 gap-1.5`}
        style={{ border: `1px solid ${error ? "rgba(239,68,68,0.5)" : "rgba(201,168,76,0.18)"}`, background: "rgba(255,255,255,0.03)" }}>
        {TIMES.map((t) => {
          ...
          return (
            <button key={t}
              ...
              aria-pressed={active}
              aria-label={`Select ${t}`}
              className="px-3 py-2.5 rounded-xl text-xs font-sans font-semibold tracking-wide transition-all duration-200
                         focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C9A84C]
                         hover:-translate-y-px"
              style={{
                background: active ? "linear-gradient(135deg,#C9A84C,#E8D5A3)" : "transparent",
                color: active ? "#080604" : "rgba(237,224,196,0.5)",
                border: active ? "none" : "1px solid rgba(201,168,76,0.1)",
              }}>
              {t}
            </button>
          );
...
/* ─── Step 2 component ───────────────────────────────────────────────────── */
function Step2DateTimePicker({ prefersReduced, form, setForm, errors, setErrors, back, next }: {
  prefersReduced: boolean | null;
  form: any; setForm: any; errors: any; setErrors: any;
  back: () => void; next: () => void;
}) {
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        {/* Calendar */}
        <div>
          <p className="block text-[9px] uppercase tracking-[0.25em] font-sans font-medium mb-1.5"
            style={{ color: "rgba(201,168,76,0.5)" }}>
            Appointment Date <span aria-hidden="true">*</span>
          </p>
          <CustomCalendar
            value={form.date}
            onChange={(v) => { setForm((p: any) => ({ ...p, date: v })); setErrors((p: any) => ({ ...p, date: "" })); }}
            error={errors.date}
          />
        ...
        <div>
          ...
          <CustomTimePicker
            value={form.time}
            onChange={(v) => { setForm((p: any) => ({ ...p, time: v })); setErrors((p: any) => ({ ...p, time: "" })); }}
            error={errors.time}
          />
          ...
          {(form.date || form.time) && (
            <div className="mt-4 rounded-xl px-4 py-3 flex items-center gap-3"
              ...
              <Calendar className="w-4 h-4 shrink-0" style={{ color: "rgba(201,168,76,0.6)" }} aria-hidden="true" />
              <p className="text-xs font-sans" style={{ color: "rgba(237,224,196,0.6)" }}>
                {form.date && <span style={{ color: "rgba(201,168,76,0.9)" }}>{form.date}</span>}
                {form.date && form.time && <span style={{ color: "rgba(237,224,196,0.3)" }}> · </span>}
                {form.time && <span style={{ color: "rgba(201,168,76,0.9)" }}>{form.time}</span>}
              </p>
            </div>
          )}
        </div>
